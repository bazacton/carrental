...
function display_tweets($id, $style = '', $max_tweets = 10, $max_cache_tweets = 10, $time = 60) {
    ...
    if (!empty($tweets)) {
        ...
        foreach ($tweets as $tweet) {
            ...
            $twitter .= "<article><div class='infotext'><i class='icon-twitter6'></i>".$id."</div><p>" . $tweet . "</p>";

            if (isset($style)) {
                if (!empty($style)) {
                    $when = ($style == 'time_since' ? '' : '');
                    $twitter.="<div class='infotext'><span> ".$when."";
                    ...
                    $twitter .= "</span></div></article>"; //end of List
                }
            }
	...
	echo '<div class="tweets-slider">'.$twitter.'</div>'; 
    ...
    if (!empty($tweets)) {
        ...
        foreach ($tweets as $tweet) {
            ...
            $tweet = preg_replace("/@([0-9a-zA-Z_-]+)/", "<a target='blank' title='$1' href=\"http://twitter.com/$1\">@$1</a>", $tweet);
									...
									$twitter .= "<a href='https://twitter.com/".$username."'>@" . $username . "</a>";
