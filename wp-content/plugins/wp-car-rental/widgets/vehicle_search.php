    ...
    function widget($args, $instance) {
        ...
        if (isset($cs_type_data) && is_array($cs_type_data) && !empty($cs_type_data)) {
            ...
            foreach ($cs_type_data as $key => $type) {
                ...
                echo '<li class="' . $active . '">' . $image . '<input name="vehicle-type" ' . $checked . ' type="radio" id="type_' . $key . '" value="' . $key . '" /><label for="type_' . $key . '">' . $cs_vehicle_type . '</label></li>';
            }
        }
        ?>
                            </ul>
...
//add_action('widgets_init', create_function('', 'return register_widget("vehicle_search");'));
 add_action('widgets_init', function() {
        return register_widget("vehicle_search");
    });
?>